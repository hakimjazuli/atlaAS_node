...
export class _FileServer {
	...
	static file_handler = (filename, force_download = false) => {
		_FileServer.caching(__Settings.__.use_caching[1]);
		if (force_download) {
			_FileServer.download_force(filename);
		}
		const fileStream = createReadStream(filename);
		__Response.__.response.setHeader('Content-Type', 'application/octet-stream');
	...
	//  * @param {string} filename
	//  */
	// static header_file_type = (filename) => {
	// 	let file_size;
	// 	try {
